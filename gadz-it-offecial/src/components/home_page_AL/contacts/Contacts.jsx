...
import { ToggleContactDetailsOpen, ToggleContactDetailsClose } from '../../../redux/slices/app';
...
const ContactListItem = ({ item, id, addMember, onAddMember, dispatch }) => {
  ...
  return (
    <ListItem sx={{ padding: '0', width: '100%', borderBottom: '0.8px solid #ab0cdf' }}>
      <ListItemButton
        onClick={() => {
          dispatch(ToggleContactDetailsOpen());
        }}
        sx={{ padding: '16px 32px', ':hover': { backgroundColor: '#2b0338' } }}
      >
        <ListItemAvatar>
...
const ContactList = ({ list, value, dispatch }) => {
...
export default function Contacts() {
  ...
  const { contactDetails } = useSelector((store) => store.app);
      ...
      <Box className="divflex-auto2" sx={{ width: contactDetails.open ? 'calc(100% - 640px)' : '100%', transition: 'width 0.5s' }}>
        ...
        >
          ...
          <Grid item xs={12} md={12} sx={{ width: "100%" }}>
            <div>
              <ContactList list={filteredContacts} value={searchInput} dispatch={dispatch} />
            </div>
          </Grid>
        </contactContext.Provider>
      </Box>
      {contactDetails.open && (
        <ContactDetails onClose={ToggleContactDetailsClose} />
      )}
    </Box>
  );
}
